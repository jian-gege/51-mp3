...
/***********************************************************************************************
 功能：将UNICODE码转为GB2312码（对于汉字来说，OEM码就是GB2312）
 形参：oem_code:OEM码 uni_code:指向用于记录oem_code所对应的UNICODE编码的变量的指针
 返回：运行结果  成功或失败
 详解：OEM码是各地区的计算机生产商在计算机固化的本地文字编码，比如在中国OEM码就是GB2312码，即区
       位码，但是FAT32的长文件名是以UNICODE来编码的，因此需要一个编码转换的过程。本函数的实现基于
       二分搜索，可快速查找到UNICODE码对应的OEM编码。
************************************************************************************************/
#ifdef USE_LFN
static UINT8 UNI2OEM(UINT16 uni_code,UINT16 *oem_code) //通过二分法查表将UNI码转为OEM码
{
	UINT32 low=0,high=MAX_UNI_INDEX-1,mid;//置当前查找区间上下界的初值 

	if(uni_code<GET_PGM_WORD(&(uni_oem[0][0]))) //判断UNI码是否在表里
		return ERR_FAIL;
	//如果输入的oem_code不是表的范围内，则直接返回
...
static UINT8 file_Open_unistr2oemstr(UINT16 *oemstr)
{
	UINT16 len=WStringLen(oemstr);	//求出长度
	UINT16 buff[MAX_LFN_LEN];		//定义相同长度数组
	...
	for(i=0;i<len;i++)
	{
		...
		if(oem_H == 0) //检查是否是ASCII码，ASCII码的数值范围为0X00~0X7F，OEM编码值不在此范围，以此区分是ASCII还是OEM
		{
			oemstr[pos] = oem_L;
		}
		else //不是ASCII码，而是OEM编码
